        ...
        <div v-if="currentStep === 1" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label v-for="c in data.collections" :key="c.id" class="cursor-pointer">
              <input :id="'c' + c.id" type="radio" name="col" :value="c.id" v-model="selectedCollectionId"
                class="sr-only" />
              <div :class="[
                ...
                <div class="flex justify-between items-start">
                  ...
                  <svg v-if="selectedCollectionId === c.id" class="w-6 h-6 text-blue-600" fill="none"
          ...
          <button v-else @click="saveJob"
            ...
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V8" />
            </svg>
            Lưu Job
          </button>
        </div>
